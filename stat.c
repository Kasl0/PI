...
void mean_variance(const double v[], double *mean, double *variance, int n)
{
    ...
    else
    {
        ...
        *variance = 0;
        for(int i=0;i<n;i++) *variance = *variance + (v[i] - *mean)*(v[i] - *mean);
        *variance /= n;
    }
}
...
{
    for(int i=0;i<n;i++) v[i] = (rand()%100 < bernoulli_percentage);
}
...
{
    for(int i=0;i<trials;i++) v[rand()%6 + rand()%6]++;
    for(int i=0;i<11;i++)
    {
        v[i] *= 100;
        v[i] /= trials;
...
void calculate_cdf(double cdf[], int trials)
{
    ...
    {
        for(int j=(rand()%6 + rand()%6);j<11;j++) cdf[j]++;
    }
    for(int i=0;i<11;i++)
    {
        cdf[i] *= 100;
...
void monty_hall(int *wins, double *win_probability, int n) {
	*wins = 0;
    for(int i = 0; i < n; i++) {
        ...
        for(int i=0;i<3;i++)
        {
            if (i != choice && i != reveal) remaining = i;
        }
