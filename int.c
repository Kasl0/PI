...
double int_left_rectangle(double x1, double x2, int n, f_1d f)
{
	double sum = 0;
	double jump = (x2-x1)/n;
	for(int i=0;i<n;i++) sum += f(x1+i*jump)*jump;
	return sum;
}
...
double int_right_rectangle(double x1, double x2, int n, f_1d f)
{
	double sum = 0;
	double jump = (x2-x1)/n;
	for(int i=0;i<n;i++) sum += f(x1+(i+1)*jump)*jump;
	return sum;
}
...
double int_mid_rectangle(double x1, double x2, int n, f_1d f)
{
	double sum = 0;
	double jump = (x2-x1)/n;
	for(int i=0;i<n;i++) sum += f(x1+i*jump+jump/2)*jump;
	return sum;
}
...
double int_trapezoid(double x1, double x2, int n, f_1d f)
{
	double sum = 0;
	double jump = (x2-x1)/n;
	for(int i=0;i<n;i++) sum += (f(x1+i*jump)+f(x1+(i+1)*jump))/2*jump;
	return sum;
}
		...
		f_2d f, boundary_2d boundary)
{
	double sum = 0;
	double not_sum = 0;
	double jump_x = (x2-x1)/nx;
	double jump_y = (y2-y1)/ny;
	for(int i=0;i<nx;i++) for(int j=0;j<ny;j++)
	if(boundary(x1+i*jump_x,y1+j*jump_y)) sum += f(x1+i*jump_x,y1+j*jump_y)*jump_x*jump_y;
	return sum;
}
		...
		double z1, double z2, int nz, f_3d f)
{
	...
	double jump_x = (x2-x1)/nx;
	double jump_y = (y2-y1)/ny;
	double jump_z = (z2-z1)/nz;
	for(int i=0;i<nx;i++) for(int j=0;j<ny;j++) for(int k=0;k<nz;k++) sum += f(x1+i*jump_x,y1+j*jump_y,z1+k*jump_z)*jump_x*jump_y*jump_z;
	return sum;
}
